...
contract Forum {
    ...
    uint256 public postIdIncreament = 1;
    uint256 public pollIdIncreament = 1;
    uint256 public commentIdIncreament = 1;
    ...
    mapping(address => uint256[]) private userPolls; //assign also address to a list of postid  
    ...
    function createPost(string memory _title, string memory _description, bool _spoil) public returns (uint256) {
        uint256 postId = postIdIncreament++;
    ...
    function createPoll(uint256 _postId, string memory _question, string memory _option1, string memory _option2)
        ...
        returns (uint256)
    {
       require(_postId <= postIdIncreament, "Post does not exist!");
        uint256 pollId = pollIdIncreament++;
    ...
    function getPoll(uint256 _pollId) public view returns (Poll memory) {
        require(_pollId <= pollIdIncreament, "Poll does not exist!");
    ...
    function getComment(uint256 _commentId) public view returns (Comment memory) {
        require(_commentId <= commentIdIncreament, "Comment does not exist!");
    ...
    function upVoteComment(uint256 _commentId) public {
        require(_commentId <= commentIdIncreament, "Post does not exist!");
